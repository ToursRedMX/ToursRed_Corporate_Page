...
export async function generateStaticParams() {
  ...
  for (const locale of locales) {
    const slugs = getAllPostSlugs('press', locale);
    slugs.forEach((slug) => {
...
export default async function PressPostPage({
  ...
  const post = getPostBySlug('press', lang as Locale, slug);
            ...
            <div className="flex flex-wrap items-center gap-4 text-slate-600 mb-6">
              ...
              {post.frontmatter.author && (
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {dict.press.by} {post.frontmatter.author}
                  </span>
                </div>
              )}
            </div>
